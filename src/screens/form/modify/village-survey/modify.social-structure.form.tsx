...
import { ISocialStructure, defaultSocialStructureData } from "./data";
import { socialStructureSchema } from "./validation";

export const ModifySocialStructureForm = () => {
  const queryClient = useQueryClient();
  // Fetch metadata using useQuery
  const {
    data: InitialSocialStructureData,
    isLoading,
    error,
    isSuccess,
  } = useQuery<ISocialStructure>({
  ...
  const {
    ...
    defaultValues: InitialSocialStructureData || defaultSocialStructureData,
    resolver: yupResolver(socialStructureSchema),
  });

  const mutation = useMutation({
    mutationKey: ["modifiedSocialStructure"],
    mutationFn: async (formData: ISocialStructure) => {
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div>
        ...
        <Controller
          ...
          render={({ field }) => (
            <>
              <input type="number" id="politicianCount" {...field} />
      ...
      <div>
        ...
        <Controller
          ...
          render={({ field }) => (
            <>
              <input type="text" id="hospital" {...field} />
      ...
      <div>
        ...
        <Controller
          ...
          render={({ field }) => (
            <>
              <input type="number" id="loanCustomers" {...field} />
              {errors.amenities?.financialServices?.microfinance
                ?.loanCustomers && (
                <p>
                  {
                    errors.amenities.financialServices.microfinance
                      .loanCustomers.message
                  }
                </p>
              )}
            </>
      ...
      <div>
        ...
        <Controller
          ...
          render={({ field }) => (
            <>
              <input type="text" id="loanProvider" {...field} />
              {errors.amenities?.financialServices?.microfinance
                ?.loanProvider && (
                <p>
                  {
      ...
      <div>
        ...
        <Controller
          ...
          render={({ field }) => (
            <>
              <select id="repaymentHistory" {...field}>
